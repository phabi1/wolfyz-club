...
export const DATAGRID_CELLS = new InjectionToken<Record<string, () => Promise<Type<Cell<any>>>>>(
  'DATAGRID_CELLS',
);

export function provideDatagrid(cells: Record<string, () => Promise<Type<Cell<any>>>>) {
  const defaultCells = {
    ...
    amount: () => import('./table/cell/amount/amount').then((m) => m.Amount),
  };
